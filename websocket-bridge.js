...
wss.on("connection", (ws, req) => {
  ...
  console.log(`📡 New WebSocket connection: ${login}@${host}:${port}`);
  ...
  tcp.on("data", (data) => {
    ...
    console.log(`📥 Received from cluster: ${clean.substring(0, 100)}${clean.length > 100 ? '...' : ''}`);
  ...
  tcp.on("close", () => {
    console.log(`🔌 TCP connection closed for ${login}@${host}:${port}`);
    try { if (ws.readyState === WebSocket.OPEN) ws.close(); } catch {}
  });

  ws.on("message", (msg) => {
    const text = msg.toString();
    console.log(`📤 Sending to cluster: ${text}`);
...
server.listen(PORT, () => {
  console.log(`🚀 WebSocket–Telnet bridge listening on port ${PORT}`);
